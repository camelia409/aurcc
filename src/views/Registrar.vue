<template>
  <div class="bg-indigo-100 mx-auto p-9">
    <h1 class="text-2xl md:text-3xl font-semibold mb-8 text-center">University Administration</h1>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
      <div 
        ...
        <div class="flex-grow flex flex-col justify-center p-6 bg-yellow-300 text-center w-full">
          <h2 class="text-xl md:text-2xl font-semibold text-black">{{ member.name }}</h2>
          <p class="text-lg text-gray-800">{{ member.position }}</p>
          <p class="text-md text-gray-800">{{ member.university }}</p>
        </div>
      </div>
    </div>
  </div>
